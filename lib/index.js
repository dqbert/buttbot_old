...
const commands = ["/commands", "/keyword", "/dkeyword", "/keywords", "/suggest", "/setgame", "/setavatar", "/setnickname"];
const commandUsage = [
      "Display this list of commands"
      ,"Add a keyword for the bot to respond to"
			,"Delete a keyword previously defined"
			,"List all keywords"
			,"Suggest a feature to be implemented for the bot to use"
      ,"Set the game which the bot is currently playing"
      ,"Set the avatar of the bot to a picture URL"
...
bot.on('message', msg => {
    ...
    if (msg.content.indexOf('/keyword ') === 0) {
          ...
          replmsg = replmsg.concat(os.EOL);
        	fs.writeFile('lib/keyword/'+msg.guild.id+'/'+usrmsg.toLowerCase()+'['+kepdel.toLowerCase()+']', replmsg, { flag: 'a' },function (err, data) {
        		if (err) {
  			 	...
  			 	sendNotif.emit('sendmsg', msg.channel, "Keyword successfully added: " + usrmsg + ". " + kepdel.toLowerCase() + " the message, then say " + replmsg);
  			}
  		}
  		else  {
  			sendNotif.emit('sendmsg', msg.channel, "usage: /keyword \"keywords\" keep/delete"/*/edit*/+" text to respond"/*/edit*/ + " with");
      ...
      fs.access('/mnt/c/Users/dqber/Documents/buttbot/lib/ideas.txt', fs.constants.W_OK, function (err) {
         if (err) {
          return console.log(err);
         }
        fs.writeFile('/mnt/c/Users/dqber/Documents/buttbot/lib/ideas.txt', suggestionMsg, {flag:'a'}, function(err, data) {
          if (err) 
            return console.log(err);
    ...
    else if (msg.content.indexOf('/setgame') === 0) {
      var cmd = msg.content;
      var game = cmd.split(" ");
      if (game[1] == null) {
        sendNotif.emit('sendmsg', msg.channel, "usage: /setgame [game]");
        return;
      }
      var completeGame = "";
      for (var i = 1; i < game.length; i++) {
        completeGame = completeGame + " " + game[i];
      }
      bot.user.setGame(completeGame)
      .then(function () { console.log("change game success");})
      .catch(function (reason) { console.log("change game failed: " + reason);});
    }
    else if (msg.content.indexOf('/setavatar') === 0) {
      var cmd = msg.content;
      var avatar = cmd.split(" ");
      if (avatar[1] == null) {
        sendNotif.emit('sendmsg', msg.channel, "usage: /setavatar [URL to an image]");
        return;
      }
      var completeAvatar = "";
      completeAvatar = avatar[1];
      bot.user.setAvatar(completeAvatar)
      .then(function () { console.log("change avatar success");})
      .catch(function (reason) { console.log("change avatar failed: " + reason);});
    }
    else if (msg.content.indexOf('/setnickname') === 0) {
      var cmd = msg.content;
      var nickname = cmd.split(" ");
      if (nickname[1] == null) {
        sendNotif.emit('sendmsg', msg.channel, "usage: /setnickname [nickname]");
      }
      var completeNickname = "";
      for (var i = 1; i < nickname.length; i++) {
        completeNickname = completeNickname + " " + nickname[i];
      }
      if (msg.guild.members.has(bot.user.id)) {
        msg.guild.members.get(bot.user.id).setNickname(completeNickname)
        .then(function () { console.log("change nickname success");})
        .catch(function (reason) { console.log("change nickname failed: " + reason);});
      }
    }
    
    //begin admin only commands
  	else if (msg.member.roles.has(adminroles.get(msg.guild.id))) { 
      ...
      fs.readdir('lib/keyword/'+msg.guild.id, 'utf8', function (err, files) {
        ...
        for (var i = 0; i < files.length; i++) {
          ...
          var channel = msg.channel;
      		//parsed[0] = message, parsed[1] = keep/delete/edit
              ...
              var replacegex = new RegExp(parsed[0], "gi");
              mesg = mesg.replace(/(\r\n|\n|\r)/gm,"");
              mesg = mesg.replace(/\s+$/, "");
      				var totalmessage = msg.content.replace(replacegex, mesg);
      				msg.delete()
      				.then(function (message) { sendNotif.emit('delsuc', message); })
      				.catch(function (reason) { sendNotif.emit('delerr', reason); });
              var totalembed = new Discord.RichEmbed({
                description: totalmessage
              });
              totalembed.setAuthor(msg.member.displayName, msg.author.displayAvatarURL, "http://niceme.me");
              totalembed.setColor(msg.member.highestRole.color);
              msg.channel.sendEmbed(totalembed)
              .then(function (message) { sendNotif.emit('sendsuc', "an embed");})
              .catch(function (reason) { sendNotif.emit('senderr', message);});
            }
      			else
...
process.stdin.on('readable', function () {
	...
	if (input.search("exit") >= 0) {
      ...
      if (channel == null)
        ...
        console.log("you must specify something to say");
      else if (saymsg.search("@") >= 0)
        console.log("@ messages are not allowed");
      else {
        bot.channels.forEach( function (chan, key, map) {
          if (chan.type == "text" && channel != null) {
            if (chan.name.search(channel) >= 0) 
