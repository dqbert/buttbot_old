...
bot.login(cfg.token).then(() => {
	...
	var keys = bot.guilds.keyArray();
	for (var i = 0; i < keys.length; i++) {
		var curguild = guilds.get(keys[i]);
		var roles = curguild.roles;
		...
		for (var j = 0; j < rolekeys.length; j++) {
			...
			if (currole.name.toLowerCase() === "buttbot-admin")
				adminroles.set(keys[i], rolekeys[j]);
		}
	}
  	/*var chans = bot.channels.array();
	for (var num=0; num<chans.length; num++) {
		if (chans[num].type=="text") {
			sendNotif.emit('sendmsg', chans[num], "Connected");
		}
	}*/
});
...
process.stdin.on('readable', function () {
	...
	if (input.search("exit")>=0) {
		...
		/*var chans = bot.channels.array();
		var mesg = input.split("exit")[1];
		var total = 0;
		var done = 0;
		var listener = new EventEmitter();
		listener.on('done', function (num1, num2) {
			if (num1>=num2) {
				bot.destroy()
				.then( function () {
		...
		for (var num=0; num<chans.length; num++) {
			if (chans[num].type=="text") {
				total++;
				if (mesg.search(".")>=0)
					chans[num].sendMessage("Disconnecting. Reason: " + mesg)
					.then(function (message) {
						sendNotif.emit('sendsuc', message); 
						done++;
						listener.emit('done', done, total);
					})
					.catch(function (reason) { 
						sendNotif.emit('senderr', reason); 
						done++;
						listener.emit('done', done, total);
					});
				else
					chans[num].sendMessage("Disconnecting.")
					.then(function (message) {
						sendNotif.emit('sendsuc' , message);
						done++;
						listener.emit('done', done, total);
		...
		var channel;
		var saymsg;
		if (input.search("channel")>=0) {
			channel=parsed[1].split("channel ")[1].split(" ")[0];
